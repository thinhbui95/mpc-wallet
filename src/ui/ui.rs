use crate::wallet::{ wallet_manager::*, interact::* };
        ...
        siv.add_layer(
            Dialog::around(form)
                ...
                .button("Create", |s| {
                    ...
                    match wallet::create_and_split_mnemonic(shares, threshold, None) {
                        Ok((mnemonic, _share_strings)) => {
                            let address = wallet::get_ethereum_address(&mnemonic).unwrap_or_default();
        ...
        siv.add_layer(
            Dialog::around(form)
                ...
                .button("Reconstruct", |s| {
                    ...
                    match wallet::reshare_mnemonic(shares, threshold, true) {
                            ...
                            move |s| {
                                ...
                                std::thread::spawn(move || {
                                    let fut = async move {
                                        ...
                                        let mnemonic = match wallet::load_and_reconstruct_mnemonic() {
                                            Ok(w) => w,
                                            Err(e) => {
                                        ...
                                        let private_key = match wallet::get_private_key_from_mnemonic(&mnemonic) {
                                            Ok(pk) => pk,
                                            Err(e) => {
                                                cb_sink.send(Box::new(move |siv| {
                                ...
                                std::thread::spawn(move || {
                                    let fut = async move {
                                        ...
                                        let wallet: LocalWallet = private_key
                                                .parse::<LocalWallet>().unwrap()
                                                .with_chain_id(chain_id.as_u64());
                                                match native::transfer_native(provider, wallet, &address, amount.into()).await {
                                                    Ok(receipt) => {
                                                        let tx_hash = receipt.transaction_hash;
                                                        let msg = format!(
